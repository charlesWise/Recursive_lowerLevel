...
class SourceStore extends ScreenComponent {
    ...
    _renderItems(level = 0, subData = this.state.sourceData) {
        this.state.storeArr.push(<Item key={subData.id}
            itemData={subData}
            getStoreInfo={(id, storeName, isEnd, isSelect) => this._getStoreInfo(id, storeName, isEnd, isSelect)}
    ...
    // 勾选
    _repeatSourceData(id, subData, isSelect) {
        if(subData.id == id) {
            subData.isSelect = isSelect;
        }
        if(subData['sub']&&subData['sub'].length > 0) {
            for(let i = 0; i < subData['sub'].length; i++) {
                this._repeatSourceData(id, subData['sub'][i], isSelect);
            }
        }
    }
    _getStoreInfo(id, storeName, isEnd, isSelect) {
        this._repeatSourceData(id, this.state.sourceData, isEnd, isSelect);
        this.props.getStoreInfo&&this.props.getStoreInfo(id, storeName, isEnd, this.state.sourceData);
    }
    render() {
        return (
...
class Item extends ScreenComponent {
    constructor(...props) {
        super(...props);
        this.state = {
            isSelect: (this.props.itemData&&this.props.itemData.isSelect) ? this.props.itemData.isSelect : false,
        }
    }
    _selectStore(itemData) {
        this.setState({
            ...
            this.props.getStoreInfo&&this.props.getStoreInfo(itemData.id, itemData.name, itemData.is_end, this.state.isSelect);
        })
    }
    _openLowerLevel(curLevel, curMarkindex) {
            ...
            <li className={`parent-list ${!itemData.isOpen && 'dn'}`}>
                <p className="parent-column"
                    ...
                    <span className="parent-title"
                        ...
                        >
                        <i className={`${this.state.isSelect ? 'icon-cell-act': 'icon-cell'}`}></i>{itemData.name}
                    </span>
                    {
